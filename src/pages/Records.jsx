...
const Records = () => {
  ...
  useEffect(() => {
    ...
    setAvailableBooks(
      storedBooks.filter((book) => book.status === "available")
    );
  }, []);
  ...
  const getBorrowerStatus = (borrower) => {
    ...
    if (pendingBooks === 0)
      return { text: "Clear", color: "bg-green-100 text-green-800" };
    return {
      text: `${pendingBooks} books pending`,
          ...
          <div className="p-6">
            ...
            <div className="text-right">
              <p className="text-lg font-semibold">
                Total Bill: ${user.totalBill.toFixed(2)}
              </p>
            </div>
          </div>
        </div>
  ...
  const AddUserModal = ({ onClose }) => {
    ...
    const calculateTotalBill = (selectedBookIds) => {
      return selectedBookIds.reduce((total, bookId) => {
        const book = availableBooks.find((b) => b.id === bookId);
        return total + (book ? parseFloat(book.price) : 0);
      }, 0);
    };

    const formatBorrowedBooks = (selectedBookIds) => {
      const today = new Date();
      const returnDate = new Date(today.setMonth(today.getMonth() + 1))
        .toISOString()
        .split("T")[0];

      return selectedBookIds.map((bookId) => {
        const book = availableBooks.find((b) => b.id === bookId);
        return {
          id: bookId,
          title: book.title,
    ...
    const handleSubmit = async (e) => {
      ...
      try {
        const borrowerData = {
          ...
          totalBill: calculateTotalBill(newUser.selectedBooks),
          borrowedBooks: formatBorrowedBooks(newUser.selectedBooks),
        };
        ...
        if (response.ok) {
          const addedBorrower = await response.json();

          // Update the books status to borrowed
          for (const bookId of newUser.selectedBooks) {
            const book = availableBooks.find((b) => b.id === bookId);
            if (book) {
              await fetch(`${API_URL}/bookes/${bookId}`, {
                method: "PUT",
                headers: {
                  "Content-Type": "application/json",
                },
          ...
          // Update local state
          setBorrowers((prev) => [...prev, addedBorrower]);

          // Refresh available books
          const booksResponse = await fetch(`${API_URL}/bookes`);
          const booksData = await booksResponse.json();
          setAvailableBooks(
            booksData.filter((book) => book.status === "available")
          );
  ...
  const handleDeleteUser = (userId) => {
    if (window.confirm("Are you sure you want to delete this user?")) {
      ...
      const updatedBooks = JSON.parse(localStorage.getItem("books")).map(
        (book) => {
  ...
  const EditUserModal = ({ user, onClose }) => {
    ...
    const handleSave = () => {
      ...
      const updatedBooks = storedBooks.map((book) => {
        const borrowedBook = editedUser.borrowedBooks.find(
          (b) => b.id === book.id
        );
        if (borrowedBook) {
          return {
            ...book,
            status: borrowedBook.returned ? "available" : "borrowed",
          };
        }
        return book;
      });
  ...
  const filteredBorrowers = borrowers.filter((borrower) => {
    ...
    const hasPendingBooks = borrower.borrowedBooks.some(
      (book) => !book.returned
    );
        ...
        <div className="mb-8 flex justify-between items-center">
          <div>
            ...
            <p className="mt-2 text-gray-600">
              Overview of all library members and their borrowed books
            </p>
          </div>
          <button
